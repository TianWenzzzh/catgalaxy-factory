...
22 个测试文件、720 条用例：CSV 解析、校验规则、星位映射、模板注入、打包、图片处理、普查解析、归并、视觉哈希（感知指纹 + 无结构图拒发指纹）、标定、名册编辑（改单元格 / 增行 / 删行）、并发锁、zip 限额、并行压缩、流式落盘、进度、主题、控制台前端接线、工作区清理（含保护名单）、CI 配置，以及端到端 API。核心模块（校验 / 注入 / 打包）均有单测。
