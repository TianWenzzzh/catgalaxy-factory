...
| 归并工作台 | 列出「同毛色 + 同区域 + 特征相似」的疑似重复组，人工判定同一只/不是/存疑并写理由；**不自动改名册**，判定理由进 F7 摘要。组内再用感知哈希按「长得像不像」预排序，人从最像的一对看起——视觉分只决定先后，**不掺进可疑度** | F9 |
| 底图标定 | 上传校园平面图后拖星星到真实位置，坐标以 0~1 归一化存储，可一键退回算法推导；页脚会注明人工标定了几颗。整片偏移不用逐颗拖——**Shift+拖拽**一颗星，它所在出没分区全体一起平移 | F8 |
...
23 个测试文件、726 条用例：CSV 解析、校验规则、星位映射、模板注入、打包、图片处理、普查解析、归并、视觉哈希（感知指纹 + 无结构图拒发指纹）、标定（含批量分区平移）、名册编辑（改单元格 / 增行 / 删行）、并发锁、zip 限额、并行压缩、流式落盘、进度、主题、控制台前端接线、工作区清理（含保护名单）、CI 配置，以及端到端 API。核心模块（校验 / 注入 / 打包）均有单测。
...
│  ├─ phash.py             F9  高通 dHash 感知指纹：给候选组做「谁跟谁最像」预排序
...
│  └─ shot.py                  截图驱动：headless Edge + CDP，七张实证图一键复现
├─ tests/                      23 个测试文件、726 条用例
├─ docs/
│  ├─ 跨校落地手册.md            普查到成品的完整 SOP + 脏 CSV 逐码处置清单
│  ├─ 验收证据-*.md             验收脚本产出的实测证据（浏览器实测含 §8 截图方法学）
│  └─ 截图/                     七张浏览器实证图（scripts/shot.py 可复现）
...
| POST | `/api/projects/{pid}/roster/rows` | F10 在第 `after` 行后插一行；**不替用户编编号**，留空就报 `E_BAD_ID` |
| DELETE | `/api/projects/{pid}/roster/rows/{line}` | F10 删掉一个物理行（表头不可删）；**照片不动**，会以 `I_PHOTO_UNUSED` 浮出来 |
