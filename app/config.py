...
# 渲染引擎（T6 收敛）：v29=v29 全特性+懒加载（F5 起为缺省），v1=旧 717 行
# 模板（deprecated，保留一个版本周期后移除）。服务端缺省走环境变量
# STARMAP_ENGINE；单次生成可用 GenerateRequest.engine 覆盖。
ACTIVE_ENGINE = os.environ.get("STARMAP_ENGINE", "v29").strip().lower()
